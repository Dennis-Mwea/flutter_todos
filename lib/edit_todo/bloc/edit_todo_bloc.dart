...
class EditTodoBloc extends Bloc<EditTodoEvent, EditTodoState> {
  EditTodoBloc({required TodosRepository todosRepository, required Todo? initialTodo})
      : _todosRepository = todosRepository,
        super(EditTodoState(title: initialTodo?.title ?? '', description: initialTodo?.description ?? '')) {
  ...
  Future<void> _onSubmitted(EditTodoSubmitted event, Emitter<EditTodoState> emit) async {
    ...
    final todo = (state.initialTodo ?? Todo(title: '')).copyWith(title: state.title, description: state.description);
    try {
