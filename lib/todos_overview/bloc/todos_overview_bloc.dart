...
class TodosOverviewBloc extends Bloc<TodosOverviewEvent, TodosOverviewState> {
  ...
  Future<void> _onSubscriptionRequested(TodosOverviewSubscriptionRequested event, Emitter<TodosOverviewState> emit) async {
    emit(state.copyWith(status: () => TodosOverviewStatus.loading));

    await emit.forEach(
      _todosRepository.getTodos(),
      onError: (_, __) => state.copyWith(status: () => TodosOverviewStatus.failure),
      onData: (todos) => state.copyWith(todos: () => todos, status: () => TodosOverviewStatus.success),
    );
  }
  ...
  Future<void> _onFilterChanged(TodosOverviewFilterChanged event, Emitter<TodosOverviewState> emit) async {
    emit(state.copyWith(filter: () => event.filter));
  }

  Future<void> _onToggleAllRequested(TodosOverviewToggleAllRequested event, Emitter<TodosOverviewState> emit) async {
    final allAreCompleted = state.todos.every((todo) => todo.isCompleted);
